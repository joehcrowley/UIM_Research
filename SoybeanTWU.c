...
int main(int argc, char* argv[])
{
    ...
    printf("\nDoubles\n");
    //Taking the best 8
    //Precip, temp, crophist, areadamaged, germination, leaves, leafspot size, stem
    //p t c a g l lS s
    double ptWU = 0;
    double pcWU = 0;
    double paWU= 0;
    ...
    for(i=0; i<102; i++){
        if(precip[i] > 0 && temp[i] > 0){
            ptWU += totalWU[i];
        }
        if(precip[i] > 0 && cropHist[i] > 0){
            pcWU += totalWU[i];
        }
        if(precip[i] > 0 && areaDamaged[i] > 0){
            paWU += totalWU[i];
        }
        if(precip[i] > 0 && germination[i] > 0){
            pgWU += totalWU[i];
        }
        if(precip[i] > 0 && leaves[i] > 0){
            plWU += totalWU[i];
        }
        if(precip[i] > 0 && leafspotSize[i] > 0){
            plSWU += totalWU[i];
        }
        if(precip[i] > 0 && stem[i] > 0){
            psWU += totalWU[i];
        }
        ...
        if(cropHist[i] > 0 && germination[i] > 0){
            cgWU += totalWU[i];
        }
        ...
        if(areaDamaged[i] > 0 && germination[i] > 0){
            agWU += totalWU[i];
        }
        ...
        if(germination[i] > 0 && leaves[i] > 0){
            glWU += totalWU[i];
        }
        if(germination[i] > 0 && leafspotSize[i] > 0){
            glSWU += totalWU[i];
        }
        if(germination[i] > 0 && stem[i] > 0){
            gsWU += totalWU[i];
        }
    ...
    printf("Precipitation and Temperature: \t%.2lf\n", ptWU);
    printf("Precipitation and Crop History: \t%.2lf\n", pcWU);
    printf("Precipitation and Area-Damaged: \t%.2lf\n", paWU);
    printf("Precipitation and Germination: \t%.2lf\n", pgWU);
    printf("Precipitation and Leaves: \t%.2lf\n", plWU);
    printf("Precipitation and Leafspot-Size: \t%.2lf\n", plSWU);
    printf("Precipitation and Stem: \t%.2lf\n", psWU);

    printf("Temperature and Crop History: \t%.2lf\n", tcWU);
    printf("Temperature and Area-Damaged: \t%.2lf\n", taWU);
    printf("Temperature and Germination: \t%.2lf\n", tgWU);
    ...
    for(i = 0; i < 102; i++){
        if(temp[i] > 0 && cropHist[i] > 0 && areaDamaged[i] > 0){
            tcaWU += totalWU[i];
        }
        if(temp[i] > 0 && cropHist[i] > 0 && leaves[i] > 0){
            tclWU += totalWU[i];
        }
        if(temp[i] > 0 && cropHist[i] > 0 && leafspotSize[i] > 0){
            tclSWU += totalWU[i];
        }
        if(temp[i] > 0 && areaDamaged[i] > 0 && leaves[i] > 0){
            talWU += totalWU[i];
        }
        if(temp[i] > 0 && areaDamaged[i] > 0 && leafspotSize[i] > 0){
            talSWU += totalWU[i];
        }
        if(temp[i] > 0 && leaves[i] > 0 && leafspotSize[i] > 0){
            tllSWU += totalWU[i];
        }

        if(cropHist[i] > 0 && areaDamaged[i] > 0 && leaves[i] > 0){
            calWU += totalWU[i];
        }
        if(cropHist[i] > 0 && areaDamaged[i] > 0 && leafspotSize[i] > 0){
            calSWU += totalWU[i];
        }
        if(cropHist[i] > 0 && leaves[i] > 0 && leafspotSize[i] > 0){
            cllSWU += totalWU[i];
        }
        if(areaDamaged[i] > 0 && leaves[i] > 0 && leafspotSize[i] > 0){
            allSWU += totalWU[i];
        }

    }
    printf("Temp, Crop History, Area-Damaged: \t%.2lf\n", tcaWU);
    printf("Temp, Crop History, Leaves: \t%.2lf\n", tclWU);
    printf("Temp, Crop History, Leafspot-Size: \t%.2lf\n", tclSWU);
    printf("Temp, Area-Damaged, Leaves: \t%.2lf\n", talWU);
    printf("Temp, Area-Damaged, Leafspot-Size: \t%.2lf\n", talSWU);
    printf("Temp, Leaves, Leafspot-Size: \t%.2lf\n", tllSWU);

    printf("Crop History, Area-Damaged, Leaves: \t%.2lf\n", calWU);
    printf("Crop History, Area-Damaged, Leafspot-Size: \t%.2lf\n", calSWU);
    printf("Crop History, Leaves, Leafspot-Size: \t%.2lf\n", cllSWU);

    printf("Area-Damaged, Leaves, Leafspot-Size: \t %.2lf\n", allSWU);
    ...
    for(i = 0; i < 102; i++){
        if(temp[i] > 0 && cropHist[i] > 0 && leaves[i] > 0 && leafspotSize[i] > 0){
            tcllSWU += totalWU[i];
        }
    }

    printf("Temp, Crop History, Leaves, Leafspot-Size: \t%.2lf\n", tcllSWU);
